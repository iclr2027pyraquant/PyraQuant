...
#
# Adapted from ScaleDiff (NeurIPS 2025) FLUX transformer with neighborhood patch attention.
...
def apply_rotary_emb(
    ...
    if text:
        ...
        cos, sin = freqs_cis  # [S, D] or routed batch [B, S, D]
        if cos.ndim == 2:
            cos = cos[None, None]
            sin = sin[None, None]
        elif cos.ndim == 3:
            if int(cos.shape[0]) != int(x.shape[0]):
                raise ValueError(
                    "Batched rotary embeddings must match the hidden-state batch: "
                    f"{cos.shape[0]} != {x.shape[0]}"
                )
            cos = cos[:, None]
            sin = sin[:, None]
        else:
            raise ValueError(f"Unsupported rotary embedding rank: {cos.ndim}")
        cos, sin = cos.to(x.device), sin.to(x.device)
...
class FluxTransformer2DModel(
    ...
    def patchify(self, q, k, v, image_rotary_emb):
        ...
        if self.query_random_jitter:
            # Allocate the jitter buffer on the live query tensor's device: under
            # sequential CPU offload the module's own device may differ from the
            # device this block executes on.
            q_pad = torch.zeros(
                B,
                H,
    ...
    def forward(
        ...
        joint_attention_kwargs.update({'text_seq_len' : txt_ids.shape[-2]})
        if self.NPAttn == True:
            if txt_ids.ndim == 3:
                txt_ids = txt_ids[0]
            if img_ids.ndim != 2:
                raise ValueError("ScaleDiff NPA full-canvas calls require 2D img_ids")
            ids = img_ids
            image_rotary_emb = self.pos_embed(ids)
        elif img_ids.ndim == 3:
            route_batch = int(img_ids.shape[0])
            if route_batch != int(hidden_states.shape[0]):
                raise ValueError(
                    "Routed absolute img_ids must match hidden-state batch: "
                    f"{route_batch} != {hidden_states.shape[0]}"
                )
            if txt_ids.ndim == 2:
                txt_ids_batched = txt_ids[None].expand(route_batch, -1, -1)
            elif txt_ids.ndim == 3 and int(txt_ids.shape[0]) == 1:
                txt_ids_batched = txt_ids.expand(route_batch, -1, -1)
            elif txt_ids.ndim == 3 and int(txt_ids.shape[0]) == route_batch:
                txt_ids_batched = txt_ids
            else:
                raise ValueError(
                    "txt_ids must be shared 2D IDs or match the routed patch batch"
                )
            ids_batched = torch.cat((txt_ids_batched, img_ids), dim=1)
            rotary = [self.pos_embed(ids_batched[index]) for index in range(route_batch)]
            image_rotary_emb = (
                torch.stack([item[0] for item in rotary], dim=0),
                torch.stack([item[1] for item in rotary], dim=0),
            )
        else:
            if txt_ids.ndim == 3:
                txt_ids = txt_ids[0]
            if img_ids.ndim != 2:
                raise ValueError(f"img_ids must be 2D or routed-batch 3D, got {img_ids.ndim}D")
