...
For every tau the main configuration is run unchanged except for the threshold of the
relative-gap rule (2K and 4K stages), and the resulting allocation is summarised:
    ...
    for tau in args.taus:
        ...
        for stage in ("256", "512"):
            ...
            if st.get("norm") == "relgap" or "thresh" in st:
                st["thresh"] = float(tau)
        run_dir = args.output_dir / f"tau_{tau:.2f}"
        ...
        print(f"[sweep] tau={tau}: 4K computed fraction {summary[f'{tau:.2f}']['computed_fraction_4096']:.3f}, "
